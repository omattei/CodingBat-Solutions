...
public boolean cigarParty(int cigars, boolean isWeekend) {
  return 40 <= cigars && (isWeekend ? true : cigars <= 60);
}
...
public int caughtSpeeding(int speed, boolean isBirthday) {
  if (speed <= (isBirthday ? 65 : 60)) 
    return 0;
  if (speed > 60 && speed <= (isBirthday ? 85 : 80))
    return 1;
  return 2;
}
...
private boolean isWeekend(int day){ return day == 0 || day == 6; }
...
public int teaParty(int tea, int candy) {
  ...
  if (Math.max(tea, candy) >= 2 * Math.min(tea, candy)) return 2;
  return 1;
}
...
public String fizzString(String str) {
  ...
  if (str.charAt(0) == 'f') reply += "Fizz";
  if (str.charAt(str.length() - 1) == 'b') reply += "Buzz";
  if (reply.length() == 0) reply = str;
  return reply;
}
...
public int maxMod5(int a, int b) {
  return (a % 5 == b % 5) ? ((a == b) ? 0 : Math.min(a, b)) : ((a == b) ? 0 : Math.max(a, b));
}
...
public int sumLimit(int a, int b) {
  ...
  int lenASumB = String.valueOf(a + b).length();
